<?php
declare(strict_types=1);

const ROLE_ADMIN = 'administrador';
const ROLE_ESTUDIANTE = 'estudiante';

function auth_user(): ?array
{
    return $_SESSION['user'] ?? null;
}

function auth_id(): ?int
{
    $u = auth_user();
    return isset($u['id']) ? (int) $u['id'] : null;
}

function require_login(): void
{
    if (!auth_user()) {
        redirect('/login');
    }
}

function login_user(int $id, string $nombre, string $identificador, string $dashboardPath, string $rol = ''): void
{
    $_SESSION['user'] = [
        'id' => $id,
        'nombre' => $nombre,
        'identificador' => $identificador,
        'dashboard' => $dashboardPath,
        'rol' => $rol,
    ];
}

function logout_user(): void
{
    $_SESSION = [];
    if (ini_get('session.use_cookies')) {
        $p = session_get_cookie_params();
        setcookie(session_name(), '', time() - 42000, $p['path'], $p['domain'], $p['secure'], $p['httponly']);
    }
    session_destroy();
}

function usuario_coincide_estudiante(array $registro, string $usuario): bool
{
    $u = trim($usuario);
    if ($u === '') {
        return false;
    }
    if ((string) ($registro['documento'] ?? '') === $u) {
        return true;
    }
    $correo = $registro['correo'] ?? '';
    if ($correo !== '' && strcasecmp((string) $correo, $u) === 0) {
        return true;
    }

    return false;
}

function require_role(string $rolEsperado): void
{
    $u = auth_user();
    if (!$u) {
        redirect('/login');
    }
    $r = (string) ($u['rol'] ?? '');
    if ($r !== $rolEsperado) {
        redirect(dashboard_url_for_user());
    }
}

function require_gestion_admin(): void
{
    require_login();
    $u = auth_user();
    if (!$u || (string) ($u['rol'] ?? '') !== ROLE_ADMIN) {
        redirect(dashboard_url_for_user());
    }
}

function attempt_login(string $usuario, string $clave): bool
{
    $usuario = trim($usuario);
    $clave = trim($clave);
    if ($usuario === '' || $clave === '') {
        return false;
    }

    foreach (load_data('administradores') as $a) {
        if (strcasecmp((string) ($a['correo'] ?? ''), $usuario) === 0 && ($a['clave'] ?? '') === $clave) {
            login_user((int) $a['id_admin'], (string) $a['nombre'], (string) $a['correo'], 'gestion/dashboard', ROLE_ADMIN);
            return true;
        }
    }

    foreach (load_data('estudiantes') as $e) {
        if (($e['clave'] ?? '') !== $clave) {
            continue;
        }
        if (!usuario_coincide_estudiante($e, $usuario)) {
            continue;
        }
        login_user(
            (int) $e['id_estudiante'],
            trim(($e['nombre'] ?? '') . ' ' . ($e['apellido'] ?? '')),
            (string) $e['documento'],
            'estudiante/dashboard',
            ROLE_ESTUDIANTE
        );
        return true;
    }

    return false;
}

function dashboard_url_normalizada(array $u): string
{
    $path = ltrim(str_replace('\\', '/', (string) ($u['dashboard'] ?? '')), '/');
    if (str_ends_with($path, '.php')) {
        $path = substr($path, 0, -4);
    }
    if ($path === 'admin/dashboard' || str_starts_with($path, 'docente/')) {
        return url('gestion/dashboard');
    }

    return url($path);
}

function dashboard_url_for_user(): string
{
    $u = auth_user();
    if (!$u) {
        return url('login');
    }
    if (!empty($u['dashboard'])) {
        return dashboard_url_normalizada($u);
    }
    if (!empty($u['rol'])) {
        return match ((string) $u['rol']) {
            ROLE_ADMIN => url('gestion/dashboard'),
            ROLE_ESTUDIANTE => url('estudiante/dashboard'),
            default => url('login'),
        };
    }

    return url('');
}
