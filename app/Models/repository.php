...
function repo_estudiante_por_documento(string $doc): ?array
{
    ...
    if (function_exists('storage_usa_mysql') && storage_usa_mysql()) {
        $st = \App\Core\Database::pdo()->prepare(
            'SELECT * FROM estudiantes WHERE documento = ? LIMIT 1'
        );
        $st->execute([$doc]);
        $e = $st->fetch();
        if (!$e) {
...
function repo_docente_por_documento(string $doc): ?array
{
    $doc = trim($doc);
    if ($doc === '') {
        return null;
    ...
    if (function_exists('storage_usa_mysql') && storage_usa_mysql()) {
        $st = \App\Core\Database::pdo()->prepare(
            'SELECT * FROM docentes WHERE documento = ? LIMIT 1'
        );
        $st->execute([$doc]);
        $d = $st->fetch();
        if (!$d) {
