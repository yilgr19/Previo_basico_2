...
**Comportamiento actual:** el **inicio de sesión** valida contra `administradores`, `docentes` o `estudiantes` (en ese orden); tras entrar, todos usan las mismas pantallas bajo `gestion/` (dashboard, estudiantes, docentes, asignaturas, matricular, reportes).
...
**Servicio** (`app/Services/GestionAcademicaService.php`): reglas de alta/edición de estudiantes, docentes, asignaturas y matrículas.
...
| `matriculas.json` | Matrículas (desde `gestion/matricular.php` y bajas en `gestion/reportes.php`). |
...
| `gestion/estudiantes.php`, `docentes.php`, `materias.php`, `matricular.php` | `Gestion\*` | Formularios y listados en `views/gestion/`. |
...
- **JS:** `panel-carrusel.js` (carrusel del inicio del panel), `estudiantes-form.js`, `docentes-form.js`, `materias-form.js`, `matricular-form.js` (ayudas en formularios). El login se valida solo en servidor.
