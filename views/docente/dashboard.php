...
$nomDoc = trim((string) ($d['nombre'] ?? '') . ' ' . (string) ($d['apellido'] ?? ''));
$primerNombre = trim((string) ($d['nombre'] ?? 'Docente'));
$ini = strtoupper(substr((string) ($d['nombre'] ?? 'D'), 0, 1) . substr((string) ($d['apellido'] ?? ''), 0, 1));
if (strlen(trim($ini)) < 2) {
    $ini = strtoupper(substr(preg_replace('/\s+/', '', $nomDoc) ?: 'D', 0, 2));
}
$sedeNombre = sede_nombre(docente_sede_efectiva($d));
$progTxt = $d['programa'] ?? programa_label_by_id((int) ($d['id_programa'] ?? 0));
?>
<main class="mx-auto w-full max-w-7xl flex-1 px-4 pb-12 sm:px-6 lg:px-8">
  <section class="relative mb-10 overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.06)] ring-1 ring-slate-900/[0.04]">
    ...
    <div class="pointer-events-none absolute -bottom-16 -left-16 h-56 w-56 rounded-full bg-sky-200/30 blur-2xl"></div>
      ...
      <div class="flex flex-col gap-8 lg:flex-row lg:items-start lg:justify-between">
        <div class="flex min-w-0 flex-1 gap-5 sm:gap-6">
          <div class="flex h-[4.5rem] w-[4.5rem] shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-academic to-academic-dark text-lg font-bold tracking-tight text-white shadow-lg shadow-academic/25 ring-4 ring-white">
            <?= h(strlen($ini) >= 2 ? $ini : substr($ini . 'D', 0, 2)) ?>
          </div>
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
              <span class="inline-flex items-center rounded-full bg-academic/10 px-3 py-1 text-[11px] font-bold uppercase tracking-wider text-academic">Docente</span>
            </div>
            <h1 class="mt-3 text-2xl font-bold tracking-tight text-slate-900 sm:text-3xl">Bienvenido, <?= h($primerNombre) ?></h1>
            <p class="mt-3 max-w-2xl text-[15px] leading-relaxed text-slate-600">
              Gestione sus trámites institucionales desde aquí. Use <strong class="font-semibold text-slate-800">Nueva solicitud</strong> para radicar y <strong class="font-semibold text-slate-800">Mis solicitudes</strong> para seguimiento por estado.
            </p>
            <div class="mt-6 flex flex-wrap gap-3">
              <a href="<?= h(url('docente/nueva_solicitud.php')) ?>" class="inline-flex items-center gap-2 rounded-xl bg-academic px-4 py-2.5 text-sm font-semibold text-white shadow-md shadow-academic/20 transition hover:bg-academic-dark">
                <svg class="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>
                Nueva solicitud
              </a>
              <a href="<?= h(url('docente/mis_solicitudes.php?tab=activas')) ?>" class="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-academic/40 hover:bg-slate-50">
        ...
        <div class="grid w-full grid-cols-2 gap-3 sm:max-w-md sm:grid-cols-2 lg:w-[min(100%,20rem)] lg:shrink-0">
          <div class="rounded-xl border border-amber-100 bg-amber-50/90 px-3 py-3 shadow-sm">
            ...
            <p class="text-[10px] text-amber-800/70">Pendiente de gestión</p>
          </div>
          ...
          <div class="rounded-xl border border-emerald-100 bg-emerald-50/90 px-3 py-3 shadow-sm">
            ...
            <p class="text-[10px] text-emerald-800/70">Resueltas a favor</p>
          </div>
      ...
      <div class="mt-8 flex flex-col gap-3 border-t border-slate-200/80 pt-6 sm:flex-row sm:flex-wrap sm:items-center sm:gap-4">
        <div class="flex items-start gap-3 rounded-xl bg-slate-50/90 px-4 py-3 ring-1 ring-slate-200/60 sm:inline-flex sm:items-center">
          ...
          <div>
            ...
            <p class="text-sm font-medium text-slate-900"><?= h($sedeNombre) ?></p>
          </div>
        </div>
        <?php if ($progTxt !== ''): ?>
        <div class="flex items-start gap-3 rounded-xl bg-slate-50/90 px-4 py-3 ring-1 ring-slate-200/60 sm:inline-flex sm:max-w-xl sm:items-center">
          ...
          <div class="min-w-0">
            <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Carrera a la que dicta clase</p>
  ...
  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
    <a href="<?= h(url('docente/nueva_solicitud.php')) ?>" class="group rounded-xl border border-gray-100 bg-white p-6 shadow-sm ring-1 ring-slate-900/[0.03] transition hover:-translate-y-0.5 hover:shadow-md">
    ...
    <a href="<?= h(url('docente/mis_solicitudes.php?tab=activas')) ?>" class="group rounded-xl border border-gray-100 bg-white p-6 shadow-sm ring-1 ring-slate-900/[0.03] transition hover:-translate-y-0.5 hover:shadow-md">
      <div class="mb-2 flex justify-center text-academic transition group-hover:scale-105">
    ...
    <a href="<?= h(url('docente/perfil.php')) ?>" class="group rounded-xl border border-gray-100 bg-white p-6 shadow-sm ring-1 ring-slate-900/[0.03] transition hover:-translate-y-0.5 hover:shadow-md">
  ...
  <p class="mt-6 rounded-lg border border-indigo-100 bg-indigo-50/50 px-4 py-3 text-xs leading-relaxed text-indigo-950/90">
    <strong class="font-semibold text-indigo-950">Menciones:</strong> en <strong>Mis solicitudes</strong> podrá ver referencias anónimas cuando un estudiante lo cite en un trámite (vista confidencial).
  </p>
</main>
